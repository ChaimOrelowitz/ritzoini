...
function computeEndDate(startDate, dowInt, numSessions) {
  ...
  last.setDate(last.getDate() + (numSessions - 1) * 7);
...
const GROUP_LIST_SELECT = `
  ...
  total_sessions, default_duration, group_soap_notes, created_at,
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    const { internal_name, group_name, supervisor_id, start_date, end_date,
            start_time, ecw_time, total_sessions, default_duration } = req.body;
    ...
    let resolvedSessions = parseInt(total_sessions) || 8;
    let resolvedEndDate  = end_date;
    if (end_date && !total_sessions) {
      resolvedSessions = computeNumSessions(start_date, end_date, dowInt);
    } else {
...
router.patch('/:id', requireAuth, async (req, res) => {
  try {
    ...
    const adminOnly      = ['internal_name', 'supervisor_id'];
    const supervisorAllowed = ['group_name','start_date','end_date','start_time','ecw_time',
                               'total_sessions','default_duration','group_soap_notes','status'];
...
// End group → remaining sessions → group_ended
router.post('/:id/end', requireAuth, async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from('groups').update({ status: 'completed' })
