...
async function requireAuth(req, res, next) {
  ...
  // Existing Ritzoini/Peer/POC APIs are internal-only. Fail closed unless the
  // authenticated user has a valid internal profile. Future public account
  // types (for example UEZ applicants) must use their own dedicated middleware.
  ...
  if (profileError || !profile) {
    return res.status(403).json({ error: 'Internal account profile required' });
  }
